...
async function inspect() {
    ...
    if (RowError) {
        ...
        latestRows.forEach(row => {
            console.log(`ID: ${row.id}, Name: ${row.name}, OrgID: ${row.organization_id}, EmpresaID: ${row.empresa_id}`);
        });
    } else {
        console.log('Table appears empty.');
    }

    // 2. Check for Foreign Key link manually
    console.log('\n--- Testing Relationship Query ---');
    const { data: RelData, error: RelError } = await supabase
        .from('finance_receivables')
        .select('id, description, clients(name)')
        .limit(1);

    if (RelError) {
        console.error('Relationship Error:', RelError);
    } else {
        console.log('Relationship Data:', JSON.stringify(RelData, null, 2));
    }
}
